...
public interface IEmployeeController {
	...
	ResponseEntity<StandardResponse> updateSalaryByPlace(
			...
			@ApiParam("Any relevant number eg 5,10 etc") @PathVariable(name = "percentage") Double percentage);
	...
	ResponseEntity<StandardResponse> getTotalSalaryBasedOnParam(
			...
			@ApiParam("Search string(Exact Match) (BU eg -Technology) (PL eg Bangalore) (SU eg - 1 or 2)") @PathVariable(name = "value") String value);

	@GetMapping("/{place}")
	@ApiOperation(value = "Find the total salary based on input parameters.", response = StandardResponse.class)
	ResponseEntity<StandardResponse> getEmployeesBasedOnPlace(
			@ApiParam("Enter the place name to get the list of employees (Eg: Bangalore,Delhi )") @PathVariable(name = "place") String place);
